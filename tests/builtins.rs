...
fn builtins() -> Result<(),Error> {
  ...
  assert_eq![u8::count_bytes(&[5])?, 1];
  assert_eq![u16::count_bytes(&[13,12])?, 2];
  assert_eq![u32::count_bytes(&[5,6,7,8])?, 4];
  assert_eq![vec![7u8,8u8,9u8].to_bytes()?, vec!(3,7,8,9)];
  assert_eq![vec![7u32,8u32,9u32].to_bytes()?, vec!(12,0,0,0,7,0,0,0,8,0,0,0,9)];
  assert_eq![<Vec<u8>>::count_bytes(&[3,7,8,9])?, 4];
...
fn booleans() -> Result<(),Error> {
  ...
  assert_eq![vec!(true,false).byte_len(), 3];
  Ok(())
}

#[test]
fn tuples() -> Result<(),Error> {
  assert_eq![(3u8,4u8).byte_len(),2];
  assert_eq![(6u16,7u16,8u16).byte_len(),6];
  assert_eq![(3u8,4u8).to_bytes()?,vec!(3,4)];
  assert_eq![(6u16,7u16,8u16).to_bytes()?,vec!(0,6,0,7,0,8)];
  assert_eq![<(u16,u16,u16)>::count_bytes(&[0,6,0,7,0,8,200,201,202,203,204])?, 6];
  assert_eq![(6u16,7u8,8u16).to_bytes()?,vec!(0,6,7,0,8)];
  assert_eq![<(u16,u8,u16)>::count_bytes(&[0,6,7,0,8,200,201,202,203,204])?, 5];
  Ok(())
}
