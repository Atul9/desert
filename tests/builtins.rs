...
fn vectors() -> Result<(),Error> {
  ...
  {
    let xs: Vec<u8> = (0..200).map(|x| {
      (((x*137+24) as u16)%256) as u8
    }).collect();
    assert_eq![
      <Vec<u8>>::from_bytes(&xs.to_bytes()?)?,
      (xs.len()+2,xs)
...
fn nested() -> Result<(),Error> {
  ...
  {
    let xs = vec![
      211, 245, 162, 214, 25, 238, 66, 38, 34, 37, 155, 210, 230, 76, 17, 182,
      128, 200, 112, 118, 77, 62, 24, 32, 201, 100, 198, 21, 244, 182, 52, 81,
      158, 55, 43, 0, 61, 115, 92, 4, 6, 80, 197, 51, 241, 19, 131, 140, 238,
      147, 19, 43, 149, 21, 93, 56, 190, 249, 106, 15, 20, 166, 69, 201, 121,
      36, 111, 180
    ];
